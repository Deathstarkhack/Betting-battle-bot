...
MY_TELEGRAM_ID = 5925363190  # Your Telegram ID

# Ensure you are added as admin on startup
if not admin_data.find_one({"user_id": MY_TELEGRAM_ID}):
    admin_data.insert_one({"user_id": MY_TELEGRAM_ID})
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("begin_"):
        ...
        if action == "win":
            ...
            await query.edit_message_text(
                f"🏆 <b>Winner:</b> @{users.find_one({'user_id': uid1})['username']} wins {amount * 2} coins!",
                parse_mode="HTML")
        else:
            users.update_one({"user_id": uid1}, {"$inc": {"coins": amount}})
            users.update_one({"user_id": uid2}, {"$inc": {"coins": amount}})
